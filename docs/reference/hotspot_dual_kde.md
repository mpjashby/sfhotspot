...
estimates for each cell. The result has classes `hspt_dk` and `hspt_k`,
and a `method` attribute containing the comparison method used.

The output can be plotted with reasonable defaults using
[`autoplot()`](https://ggplot2.tidyverse.org/reference/autoplot.html).
The colour scale is selected according to the comparison method.
